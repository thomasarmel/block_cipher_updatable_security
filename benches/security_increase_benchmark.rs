...
use block_cipher_updatable_security::{encrypt, increase_security_level, Iv, Key};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("increase security small", |b| b.iter(|| increase_security_level(black_box(&encrypt_small), &iv, &key, &key2)));
    c.bench_function("increase security poem", |b| b.iter(|| increase_security_level(black_box(&encrypt_poem), &iv, &key, &key2)));
}
