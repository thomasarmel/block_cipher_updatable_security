    ...
    let new_cipher_blocks: Vec<EncryptedBlock> = cipher_blocks
        ...
        .map(|(new_block_count, encrypted_blocks_bytes)| {
            ...
            if encrypted_blocks_bytes.len() == 1 {
                ...
                let null_plain_block = PlainBlock::from_bytes(
                    &null_bytes,
                    encrypted_blocks_bytes[0].block_count() + 1,
                )
                .unwrap();
            ...
            let expanded_block = polyadd(
                &POLYMULTIPLIER.polymul_fast(
                    &polyadd(
                        &odd_block.enc_polynomial(),
                        &odd_block.mul_polynomial(),
                        POLYNOMIAL_Q as i64,
                        new_key.get_modulus_polynomial(),
                    ),
                    &generate_merging_block_polynomial(old_security_level),
                ...
                &polyadd(
                    &even_block.enc_polynomial(),
                    &even_block.mul_polynomial(),
                    POLYNOMIAL_Q as i64,
                    new_key.get_modulus_polynomial(),
                ),
                POLYNOMIAL_Q as i64,
        ...
        .map(|(super_block_count, temp_polynomial)| {
            ...
            let temp_key_factor = POLYMULTIPLIER.polymul_fast(
                &iv.pow(
                    (super_block_count << new_key.key_generation()) + 1,
                    new_key.get_modulus_polynomial(),
                ),
                &temp_key_polynomial,
                new_key.get_modulus_polynomial(),
...
#[cfg(test)]
mod tests {
    use crate::{Iv, Key, decrypt, encrypt, increase_security_level};
    ...
    fn test_reencryption() {
        ...
        let decrypted = decrypt(&encrypted, &key1, &iv);
        let decrypted_text =
            std::str::from_utf8(&decrypted).unwrap()[..PLAINTEXT.len()].to_string();
        ...
        let decrypted2_text =
            std::str::from_utf8(&decrypted2).unwrap()[..PLAINTEXT.len()].to_string();
    ...
    fn test_multiple_reencryptions() {
        ...
        let decrypted3_text =
            std::str::from_utf8(&decrypted3).unwrap()[..PLAINTEXT.len()].to_string();
        ...
        let decrypted4_text =
            std::str::from_utf8(&decrypted4).unwrap()[..PLAINTEXT.len()].to_string();
