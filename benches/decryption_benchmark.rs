use block_cipher_updatable_security::{Iv, Key, decrypt, encrypt};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("decrypt small", |b| {
        b.iter(|| decrypt(black_box(&encrypt_small), &key, &iv))
    });
    c.bench_function("decrypt poem", |b| {
        b.iter(|| decrypt(black_box(&encrypt_poem), &key, &iv))
    });
}
